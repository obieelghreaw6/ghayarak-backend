...
// GET /orders?role=buyer|seller — "My orders" or "My sales"
...
router.get("/:id", requireAuth, async (req, res) => {
  ...
  const [history, disputes, protection] = await Promise.all([
  ...
  res.json({ order, history: history.rows, disputes: disputes.rows, protection: protection.rows[0] || null });
});
